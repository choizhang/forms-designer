...
1. 重复表页眉页脚bugfix
2. 替换文本域和重复表的图标
3. 域结构可以平级拖动，支持数字前缀排序
4. bugfix
5. (文本,重复表)组件添加状态反射功能

### 8.18 ###
1. 自定义插入表格组件添加状态反射功能
2. 实现剪切的功能,研究粘贴扔无法解决只能用键盘
3. 将间距等多值设置变为分开设置
4. 对整个文档设置公共的字体(类型,大小,颜色)

### 8.19 ###
1. 添加多个iframe的bugfix
2. 将最新的修改发布到线上
3. fix状态反射功能后文本域结构不能自动添加的bug
4. 将'文本域'文本全部改成'域'
5. 跟吴总验收上周迭代,讨论整理下周需求点
...
3. 给文本域添加边框属性设置
4. 取消文本域预设的宽度,采取自适应
5. 重复表如果页脚是隐藏的状态,无法通过拖动边框来变化宽度的bugfix

### 8.23 ###
1. 实现了表格内非连续单元格多选的效果(可以任意批量化设置)
2. 解决文本域文字左右未顶对齐的bug
3. 表格点击第一列和第一行时实现整行选中的效果bugfix

### 8.24 ###
1. 目前的分割应改为取消合并，另外再实现任意单元格分割
2. 文本域添加上下左右间距(textarea?)
3. 表格单元格权限设置为经理之后再设置为普通无效bug

### 8.25 ###
1. 文本域的部分属性设置要运用到重复表的设置当中
