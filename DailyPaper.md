...
### 8.8 ###
1. 将域结构的刷新功能添加在特定事件的回调中以提升体验
2. 确定ueditor本身的底部提升功能的展示与否
3. 修复选择多个单元格的时候再选择设置,会将选择高亮的背景色默认为单元格背景色的bug
4. 将代码给代毅部署到测试服务器上
5. 插入表格,要控制表格可设置总宽度(表格,单元格)
6. 将域结构和组件样式设置放到一个tabset里
7. 域结构的视图概念取消了,我之前对视图权限的处理方式也做相应调整
...
视图中的域结构即使被删除了,域结构中还是应该保留以便后面再添加

插入一样的: 就只有label可以修改,如果修改了feildTitle,那么就是全部都改了

feildTitle是肯定有的,而feildName有可能没有.比如重复表,外层结构就不需要用feildName来表示

我导出的数据可以作为导入的数据来初始化系统


controlls用于创建重复表

html要做一个处理,就1种类型input.样式直接绑给input,高宽要做额外处理
